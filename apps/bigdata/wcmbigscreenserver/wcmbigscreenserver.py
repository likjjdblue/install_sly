...
class WCMBigScreenServerTool(object):
    def __init__(self, namespace='default', wcmbigscreenserverdata='wcm-bigscreen-server-pv-data',
        ...
        self.AppInfo['WCMBigScreenServerDataPath'] = os.path.join(self.AppInfo['DataStorageBasePath'], '-'.join([namespace, wcmbigscreenserverdata]))
    ...
    def setupStorage(self):
        ...
        print ('create WCMBigScreenServer Storage successfully')


        self.DataStorageObj.createSubFolder(self.AppInfo['WCMBigScreenServerDataPath'])

        self.TmpStoragePathDict = dict()
        self.TmpStoragePathDict['WCMBigScreenServerDataPath'] = self.DataStorageObj.generateRealPath(self.AppInfo['WCMBigScreenServerDataPath'])

        print ('setup WCMBigScreenServer Storage successfully')
    ...
    def generateValues(self):
        self.AppInfo['WCMBigScreenServerImage'] = replaceDockerRepo(self.AppInfo['WCMBigScreenServerImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['NFSProvisionerImage'] =replaceDockerRepo(self.AppInfo['NFSProvisionerImage'],
                                                               self.AppInfo['HarborAddr'])

        '''
        if not self.AppInfo['WCMBigScreenServerSecondDBPassword']:
            self.AppInfo['WCMBigScreenServerSecondDBPassword'] = crypto_tools.generateRandomAlphaNumericString(lenght=10)
        '''
    ...
    def applyYAML(self):
        ...
        print ('Apply WCMBigScreenServer ....')
        if not self.k8sObj.checkNamespacedResourceHealth(name='wcm-bigscreen-server-deploy', namespace=self.AppInfo['Namespace'],
                                                         kind='Deployment'):
            try:
                self.k8sObj.deleteNamespacedDeployment(name='wcm-bigscreen-server-deploy', namespace=self.AppInfo['Namespace'])
            except:
                pass
            ...
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'wcm-bigscreen-server-cm.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'wcm-bigscreen-server-svc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            ...
            TmpResponse = self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'wcm-bigscreen-server-deploy.yaml'),
                                                         namespace=self.AppInfo['Namespace'])
        ...
        for itime in range(self.RetryTimes):
            TmpResponse = self.k8sObj.getNamespacedDeployment(name='wcm-bigscreen-server-deploy',
            ...
            TmpCheckResult = TmpServiceCheckObj.checkServicePortState(targetaddress='wcm-bigscreen-server-svc:9994')
            print ('wcm-bigscreen-server-svc:9994 is listening....')
    ...
    def start(self):
        ...
        if TmpResponse['ret_code'] != 0:
            print ('failed to install WCMBigScreenServer')
            return TmpResponse
    ...
    def installDependences(self):
        for TmpRawStr in self.AppInfo['dependences']:
            ...
            if TmpClsName == 'MariaDBTool':
                ...
                self.AppInfo['WCMBigScreenServerPrimaryDBPassword'] = TmpInfo['TRSWCMDBPassword']
    ...
    def PreInstall(self):
        ...
        with open(os.path.join(self.BaseDIRPath, 'tmp', 'account.txt'),mode='wb',encoding='utf-8') as f:
            ...
            f.write('%s %s %s'%(self.AppInfo['WCMBigScreenServerPrimaryDBName'], self.AppInfo['WCMBigScreenServerPrimaryDBUser'], self.AppInfo['WCMBigScreenServerPrimaryDBPassword'])+'\n')
    ...
    def postInstall(self):
        ...
        self.DataStorageObj.uploadFile(localpath=os.path.join(self.BaseDIRPath, 'downloads', 'wcmbigscreenserver.conf'),
                                  remotepath=os.path.join(TmpNginxConfigPath, 'wcmbigscreenserver.conf')
                                  )
...
if __name__ == "__main__":
    tmp = WCMBigScreenServerTool(namespace='sly2', nfsinfo=dict(hostname='192.168.200.74', port=1022, username='root', password='!QAZ2wsx1234',
                         basepath='/TRS/DATA'))
