...
class ResourceTool(object):
    def __init__(self, namespace='default', resourcelog='resource-pv-log' , resourcedata='resource-pv-data',
                 resourcemediadata='resource-pv-mediadata',
        ...
        self.AppInfo['ResourceLogPath'] = os.path.join(self.AppInfo['LogStorageBasePath'], '-'.join([namespace, resourcelog]))
        self.AppInfo['ResourceDataPath'] = os.path.join(self.AppInfo['DataStorageBasePath'], '-'.join([namespace, resourcedata]))
        self.AppInfo['ResourceMediaDataPath'] = os.path.join(self.AppInfo['DataStorageBasePath'], '-'.join([namespace, resourcemediadata]))
    ...
    def setupStorage(self):
        ...
        print ('create Resource Storage successfully')


        self.LogStorageObj.createSubFolder(self.AppInfo['ResourceLogPath'])
        self.DataStorageObj.createSubFolder(self.AppInfo['ResourceDataPath'])
        self.DataStorageObj.createSubFolder(self.AppInfo['ResourceMediaDataPath'])


        self.TmpStoragePathDict = dict()
        self.TmpStoragePathDict['ResourceDataPath'] = self.DataStorageObj.generateRealPath(self.AppInfo['ResourceDataPath'])
        self.TmpStoragePathDict['ResourceMediaDataPath'] = self.DataStorageObj.generateRealPath(self.AppInfo['ResourceMediaDataPath'])
        self.TmpStoragePathDict['ResourceLogPath'] = self.LogStorageObj.generateRealPath(self.AppInfo['ResourceLogPath'])

        print ('setup Resource Storage successfully')
    ...
    def generateValues(self):
        self.AppInfo['ResourceImage'] = replaceDockerRepo(self.AppInfo['ResourceImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['NFSProvisionerImage'] =replaceDockerRepo(self.AppInfo['NFSProvisionerImage'],
                                                               self.AppInfo['HarborAddr'])
        if not self.AppInfo['ResourcePrimaryDBPassword']:
            self.AppInfo['ResourcePrimaryDBPassword'] = crypto_tools.generateRandomAlphaNumericString(lenght=10)
    ...
    def applyYAML(self):
        ...
        if not self.k8sObj.checkNamespacedResourceHealth(name='resource-deploy', namespace=self.AppInfo['Namespace'],
                                                         kind='Deployment'):
            try:
                self.k8sObj.deleteNamespacedDeployment(name='resource-deploy', namespace=self.AppInfo['Namespace'])
            except:
                pass
            ...
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'resource-cm.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'resource-svc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/resource-pv.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/resource-pvc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )


            TmpResponse = self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'resource-deploy.yaml'),
                                                         namespace=self.AppInfo['Namespace'])
    ...
    def start(self):
        ...
        if TmpResponse['ret_code'] != 0:
            print ('failed to install Resource')
            return TmpResponse
    ...
    def installDependences(self):
        for TmpRawStr in self.AppInfo['dependences']:
            ...
            if TmpClsName == 'MariaDBTool':
                ...
                self.AppInfo['ResourcePrimaryDBPassword'] = TmpInfo['DicttoolDBPassword']
    ...
    def PreInstall(self):
        ...
        with open(os.path.join(self.BaseDIRPath, 'tmp', 'account.txt'),mode='wb',encoding='utf-8') as f:
            ...
            f.write('%s %s %s'%(self.AppInfo['ResourcePrimaryDBName'], self.AppInfo['ResourcePrimaryDBUser'], self.AppInfo['ResourcePrimaryDBPassword'])+'\n')
    ...
    def postInstall(self):
        ...
        self.DataStorageObj.uploadFile(localpath=os.path.join(self.BaseDIRPath, 'downloads', 'resource.conf'),
                                  remotepath=os.path.join(TmpNginxConfigPath, 'resource.conf')
                                  )
...
if __name__ == "__main__":
    tmp = ResourceTool(namespace='sly2', nfsinfo=dict(hostname='192.168.200.74', port=1022, username='root', password='!QAZ2wsx1234',
                         basepath='/TRS/DATA'))
