...
class MCBMessageTool(object):
    def __init__(self, namespace='default', mcbmessagelog='mcb-message-pv-log' , mcbmessagedata='mcb-message-pv-data',
        ...
        self.AppInfo['MCBMessageLogPath'] = os.path.join(self.AppInfo['LogStorageBasePath'], '-'.join([namespace, mcbmessagelog]))
        self.AppInfo['MCBMessageDataPath'] = os.path.join(self.AppInfo['DataStorageBasePath'], '-'.join([namespace, mcbmessagedata]))
    ...
    def setupStorage(self):
        ...
        print ('create MCBMessage Storage successfully')


        self.LogStorageObj.createSubFolder(self.AppInfo['MCBMessageLogPath'])
        self.DataStorageObj.createSubFolder(self.AppInfo['MCBMessageDataPath'])


        self.TmpStoragePathDict = dict()
        self.TmpStoragePathDict['MCBMessageDataPath'] = self.DataStorageObj.generateRealPath(self.AppInfo['MCBMessageDataPath'])
        self.TmpStoragePathDict['MCBMessageLogPath'] = self.LogStorageObj.generateRealPath(self.AppInfo['MCBMessageLogPath'])

        print ('setup MCBMessage Storage successfully')
    ...
    def generateValues(self):
        self.AppInfo['MCBMessageImage'] = replaceDockerRepo(self.AppInfo['MCBMessageImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['NFSProvisionerImage'] =replaceDockerRepo(self.AppInfo['NFSProvisionerImage'],
                                                               self.AppInfo['HarborAddr'])
        if not self.AppInfo['MCBMessagePrimaryDBPassword']:
            self.AppInfo['MCBMessagePrimaryDBPassword'] = crypto_tools.generateRandomAlphaNumericString(lenght=10)
    ...
    def applyYAML(self):
        ...
        print ('Apply MCBMessage ....')
        if not self.k8sObj.checkNamespacedResourceHealth(name='mcb-message-deploy', namespace=self.AppInfo['Namespace'],
                                                         kind='Deployment'):
            try:
                self.k8sObj.deleteNamespacedDeployment(name='mcb-message-deploy', namespace=self.AppInfo['Namespace'])
            except:
                pass
            ...
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'mcb_message-cm.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'mcb_message-svc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/mcb_message-pv.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/mcb_message-pvc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )


            TmpResponse = self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'mcb_message-deploy.yaml'),
                                                         namespace=self.AppInfo['Namespace'])
        ...
        for itime in range(self.RetryTimes):
            TmpResponse = self.k8sObj.getNamespacedDeployment(name='mcb-message-deploy',
    ...
    def start(self):
        ...
        if TmpResponse['ret_code'] != 0:
            print ('failed to install MCBMessage')
            return TmpResponse
    ...
    def PreInstall(self):
        ...
        with open(os.path.join(self.BaseDIRPath, 'tmp', 'account.txt'),mode='wb',encoding='utf-8') as f:
            ...
            f.write('%s %s %s'%(self.AppInfo['MCBMessagePrimaryDBName'], self.AppInfo['MCBMessagePrimaryDBUser'], self.AppInfo['MCBMessagePrimaryDBPassword'])+'\n')
    ...
    def postInstall(self):
        ...
        self.DataStorageObj.uploadFile(localpath=os.path.join(self.BaseDIRPath, 'downloads', 'mcbmessage.conf'),
                                  remotepath=os.path.join(TmpNginxConfigPath, 'mcbmessage.conf')
                                  )
...
if __name__ == "__main__":
    tmp = MCBMessageTool(namespace='sly2', nfsinfo=dict(hostname='192.168.200.74', port=1022, username='root', password='!QAZ2wsx1234',
                         basepath='/TRS/DATA'))
