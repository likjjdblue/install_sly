...
class PushSynTool(object):
    def __init__(self, namespace='default', pushsynlog='push-syn-pv-log',
        ...
        self.AppInfo['PushSynLogPath'] = os.path.join(self.AppInfo['LogStorageBasePath'], '-'.join([namespace, pushsynlog]))
    ...
    def setupStorage(self):
        ...
        print ('create PushSyn Storage successfully')


        self.LogStorageObj.createSubFolder(self.AppInfo['PushSynLogPath'])

        self.TmpStoragePathDict = dict()
        self.TmpStoragePathDict['PushSynLogPath'] = self.LogStorageObj.generateRealPath(self.AppInfo['PushSynLogPath'])


        print ('setup PushSyn Storage successfully')
    ...
    def generateValues(self):
        self.AppInfo['PushSynImage'] = replaceDockerRepo(self.AppInfo['PushSynImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['NFSProvisionerImage'] =replaceDockerRepo(self.AppInfo['NFSProvisionerImage'],

                                                               self.AppInfo['HarborAddr'])


        if not self.AppInfo['PushSynPrimaryDBPassword']:
            self.AppInfo['PushSynPrimaryDBPassword'] = crypto_tools.generateRandomAlphaNumericString(lenght=10)
    ...
    def applyYAML(self):
        ...
        print ('Apply PushSyn ....')
        if not self.k8sObj.checkNamespacedResourceHealth(name='push-syn-deploy', namespace=self.AppInfo['Namespace'],
                                                         kind='Deployment'):
            try:
                self.k8sObj.deleteNamespacedDeployment(name='push-syn-deploy', namespace=self.AppInfo['Namespace'])
            except:
                pass
            ...
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'push-syn-cm.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'push-syn-svc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/push-syn-pv.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/push-syn-pvc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )


            TmpResponse = self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'push-syn-deploy.yaml'),
                                                         namespace=self.AppInfo['Namespace'])
        ...
        for itime in range(self.RetryTimes):
            TmpResponse = self.k8sObj.getNamespacedDeployment(name='push-syn-deploy',
            ...
            TmpCheckResult = TmpServiceCheckObj.checkServicePortState(targetaddress='push-syn-svc:58105')
            print ('push-syn-svc:58105 is listening....')
    ...
    def start(self):
        ...
        if TmpResponse['ret_code'] != 0:
            print ('failed to install PushSyn')
            return TmpResponse
    ...
    def PreInstall(self):
        ...
        with open(os.path.join(self.BaseDIRPath, 'tmp', 'account.txt'),mode='wb',encoding='utf-8') as f:
            ...
            f.write('%s %s %s'%(self.AppInfo['PushSynPrimaryDBName'], self.AppInfo['PushSynPrimaryDBUser'], self.AppInfo['PushSynPrimaryDBPassword'])+'\n')
...
if __name__ == "__main__":
    tmp = PushSynTool(namespace='sly2', nfsinfo=dict(hostname='192.168.200.74', port=1022, username='root', password='!QAZ2wsx1234',
                         basepath='/TRS/DATA'))
