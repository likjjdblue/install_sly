...
class MCBPMTool(object):
    def __init__(self, namespace='default', mcbmessagelog='mcb-pm-pv-log' , mcbmessagedata='mcb-pm-pv-data',
        ...
        self.AppInfo['MCBPMLogPath'] = os.path.join(self.AppInfo['LogStorageBasePath'], '-'.join([namespace, mcbmessagelog]))
        self.AppInfo['MCBPMDataPath'] = os.path.join(self.AppInfo['DataStorageBasePath'], '-'.join([namespace, mcbmessagedata]))
    ...
    def setupStorage(self):
        ...
        print ('create MCBPM Storage successfully')


        self.LogStorageObj.createSubFolder(self.AppInfo['MCBPMLogPath'])
        self.DataStorageObj.createSubFolder(self.AppInfo['MCBPMDataPath'])

        self.TmpStoragePathDict = dict()
        self.TmpStoragePathDict['MCBPMDataPath'] = self.DataStorageObj.generateRealPath(self.AppInfo['MCBPMDataPath'])
        self.TmpStoragePathDict['MCBPMLogPath'] = self.LogStorageObj.generateRealPath(self.AppInfo['MCBPMLogPath'])

        print ('setup MCBPM Storage successfully')
    ...
    def applyYAML(self):
        ...
        for itime in range(self.RetryTimes):
            ...
            TmpCheckResult = TmpServiceCheckObj.checkServicePortState(targetaddress='mcb-pm-svc:8095')
            print ('mcb-pm-svc:8095 is listening....')
