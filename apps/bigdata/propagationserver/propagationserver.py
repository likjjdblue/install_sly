...
class PropagationServerTool(object):
    def __init__(self, namespace='default', propagationserverlog='propagation-server-pv-log',
        ...
        self.AppInfo['PropagationServerLogPath'] = os.path.join(self.AppInfo['LogStorageBasePath'], '-'.join([namespace, propagationserverlog]))
    ...
    def setupStorage(self):
        ...
        print ('create PropagationServer Storage successfully')


        self.LogStorageObj.createSubFolder(self.AppInfo['PropagationServerLogPath'])

        self.TmpStoragePathDict = dict()
        self.TmpStoragePathDict['PropagationServerLogPath'] = self.LogStorageObj.generateRealPath(self.AppInfo['PropagationServerLogPath'])


        print ('setup PropagationServer Storage successfully')
    ...
    def generateValues(self):
        self.AppInfo['PropagationServerImage'] = replaceDockerRepo(self.AppInfo['PropagationServerImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['NFSProvisionerImage'] =replaceDockerRepo(self.AppInfo['NFSProvisionerImage'],
                                                               self.AppInfo['HarborAddr'])
        if not self.AppInfo['PropagationServerPrimaryDBPassword']:
            self.AppInfo['PropagationServerPrimaryDBPassword'] = crypto_tools.generateRandomAlphaNumericString(lenght=10)
    ...
    def applyYAML(self):
        ...
        print ('Apply PropagationServer ....')
        if not self.k8sObj.checkNamespacedResourceHealth(name='propagation-server-deploy', namespace=self.AppInfo['Namespace'],
                                                         kind='Deployment'):
            try:
                self.k8sObj.deleteNamespacedDeployment(name='propagation-server-deploy', namespace=self.AppInfo['Namespace'])
            except:
                pass
            ...
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'propagation-server-cm.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'propagation-server-svc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/propagation-server-pv.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/propagation-server-pvc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )


            TmpResponse = self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'propagation-server-deploy.yaml'),
                                                         namespace=self.AppInfo['Namespace'])
        ...
        for itime in range(self.RetryTimes):
            TmpResponse = self.k8sObj.getNamespacedDeployment(name='propagation-server-deploy',
            ...
            TmpCheckResult = TmpServiceCheckObj.checkServicePortState(targetaddress='propagation-server-svc:58101')
            print ('propagation-server-svc:58101 is listening....')
    ...
    def start(self):
        ...
        if TmpResponse['ret_code'] != 0:
            print ('failed to install PropagationServer')
            return TmpResponse
    ...
    def PreInstall(self):
        ...
        with open(os.path.join(self.BaseDIRPath, 'tmp', 'account.txt'),mode='wb',encoding='utf-8') as f:
            ...
            f.write('%s %s %s'%(self.AppInfo['PropagationServerPrimaryDBName'], self.AppInfo['PropagationServerPrimaryDBUser'], self.AppInfo['PropagationServerPrimaryDBPassword'])+'\n')
        ...
        print (os.path.join(self.BaseDIRPath, 'downloads', 'propagation.sql'))
        ...
        self.DataStorageObj.uploadFile(localpath=os.path.join(self.BaseDIRPath, 'downloads', 'propagation.sql'),
                                  remotepath=os.path.join(TmpSQLToolSQLPath, 'propagation.sql')
                                  )
    ...
    def postInstall(self):
        ...
        '''
        self.DataStorageObj.uploadFile(localpath=os.path.join(self.BaseDIRPath, 'downloads', 'piccenter.conf'),
                                  remotepath=os.path.join(TmpNginxConfigPath, 'piccenter.conf')
                                  )
        '''
...
if __name__ == "__main__":
    tmp = PropagationServerTool(namespace='sly2', nfsinfo=dict(hostname='192.168.200.74', port=1022, username='root', password='!QAZ2wsx1234',
                         basepath='/TRS/DATA'))
