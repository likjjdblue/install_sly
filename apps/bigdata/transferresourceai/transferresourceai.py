...
class TransferResourceAITool(object):
    def __init__(self, namespace='default', transferresourceailog='transfer-resource-ai-pv-log' ,
        ...
        self.AppInfo['TransferResourceAILogPath'] = os.path.join(self.AppInfo['LogStorageBasePath'], '-'.join([namespace, transferresourceailog]))
    ...
    def setupStorage(self):
        ...
        print ('create TransferResourceAI Storage successfully')


        self.LogStorageObj.createSubFolder(self.AppInfo['TransferResourceAILogPath'])

        self.TmpStoragePathDict = dict()
        self.TmpStoragePathDict['TransferResourceAILogPath'] = self.LogStorageObj.generateRealPath(self.AppInfo['TransferResourceAILogPath'])

        print ('setup TransferResourceAI Storage successfully')
    ...
    def generateValues(self):
        self.AppInfo['TransferResourceAIImage'] = replaceDockerRepo(self.AppInfo['TransferResourceAIImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['NFSProvisionerImage'] =replaceDockerRepo(self.AppInfo['NFSProvisionerImage'], self.AppInfo['HarborAddr'])

        if not self.AppInfo['TransferResourceAIPrimaryDBPassword']:
            self.AppInfo['TransferResourceAIPrimaryDBPassword'] = crypto_tools.generateRandomAlphaNumericString(lenght=10)
    ...
    def applyYAML(self):
        ...
        print ('Apply TransferResourceAI ....')
        if not self.k8sObj.checkNamespacedResourceHealth(name='transfer-resource-ai-deploy', namespace=self.AppInfo['Namespace'],
                                                         kind='Deployment'):
            try:
                self.k8sObj.deleteNamespacedDeployment(name='transfer-resource-ai-deploy', namespace=self.AppInfo['Namespace'])
            except:
                pass
            ...
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'transfer_resource_ai-cm.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'transfer_resource_ai-svc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/transfer_resource_ai-pv.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/transfer_resource_ai-pvc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )


            TmpResponse = self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'transfer_resource_ai-deploy.yaml'),
                                                         namespace=self.AppInfo['Namespace'])
        ...
        for itime in range(self.RetryTimes):
            TmpResponse = self.k8sObj.getNamespacedDeployment(name='transfer-resource-ai-deploy',
            ...
            TmpCheckResult = TmpServiceCheckObj.checkServicePortState(targetaddress='transfer-resource-ai-svc:58585')
            print ('transfer-resource-ai-svc:58585 is listening....')
    ...
    def start(self):
        ...
        if TmpResponse['ret_code'] != 0:
            print ('failed to install TransferResourceAI')
            return TmpResponse
    ...
    def PreInstall(self):
        ...
        with open(os.path.join(self.BaseDIRPath, 'tmp', 'account.txt'),mode='wb',encoding='utf-8') as f:
            ...
            f.write('%s %s %s'%(self.AppInfo['TransferResourceAIPrimaryDBName'], self.AppInfo['TransferResourceAIPrimaryDBUser'], self.AppInfo['TransferResourceAIPrimaryDBPassword'])+'\n')
    ...
    def postInstall(self):
        ...
        self.DataStorageObj.uploadFile(localpath=os.path.join(self.BaseDIRPath, 'downloads', 'transferresourceai.conf'),
                                  remotepath=os.path.join(TmpNginxConfigPath, 'transferresourceai.conf')
                                  )
...
if __name__ == "__main__":
    tmp = TransferResourceAITool(namespace='sly2', nfsinfo=dict(hostname='192.168.200.74', port=1022, username='root', password='!QAZ2wsx1234',
                         basepath='/TRS/DATA'))
