...
class DicttoolTool(object):
    def __init__(self, namespace='default', dicttooldata='dicttool-pv-log' ,
        ...
        self.AppInfo['DicttoolLogPath'] = os.path.join(self.AppInfo['LogStorageBasePath'], '-'.join([namespace, dicttooldata]))
    ...
    def setupStorage(self):
        ...
        print ('create Dicttool Storage successfully')


        self.LogStorageObj.createSubFolder(self.AppInfo['DicttoolLogPath'])

        self.TmpStoragePathDict = dict()
        self.TmpStoragePathDict['DicttoolLogPath'] = self.LogStorageObj.generateRealPath(self.AppInfo['DicttoolLogPath'])

        print ('setup Dicttool Storage successfully')
    ...
    def generateValues(self):
        self.AppInfo['DicttoolImage'] = replaceDockerRepo(self.AppInfo['DicttoolImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['NFSProvisionerImage'] =replaceDockerRepo(self.AppInfo['NFSProvisionerImage'],
                                                               self.AppInfo['HarborAddr'])
        if not self.AppInfo['DicttoolDBPassword']:
            self.AppInfo['DicttoolDBPassword'] = crypto_tools.generateRandomAlphaNumericString(lenght=10)
    ...
    def applyYAML(self):
        ...
        print ('Apply Dicttool ....')
        if not self.k8sObj.checkNamespacedResourceHealth(name='dicttool-deploy', namespace=self.AppInfo['Namespace'],
                                                         kind='Deployment'):
            try:
                self.k8sObj.deleteNamespacedDeployment(name='dicttool-deploy', namespace=self.AppInfo['Namespace'])
            except:
                pass
            ...
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'dicttool-cm.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'dicttool-svc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/dicttool-pv.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/dicttool-pvc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )


            TmpResponse = self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'dicttool-deploy.yaml'),
                                                         namespace=self.AppInfo['Namespace'])
        ...
        for itime in range(self.RetryTimes):
            TmpResponse = self.k8sObj.getNamespacedDeployment(name='dicttool-deploy',
            ...
            TmpCheckResult = TmpServiceCheckObj.checkServicePortState(targetaddress='dicttool-svc:8090')
            print ('dicttool-svc:8090 is listening....')
    ...
    def start(self):
        ...
        if TmpResponse['ret_code'] != 0:
            print ('failed to install Dicttool')
            return TmpResponse
    ...
    def PreInstall(self):
        ...
        with open(os.path.join(self.BaseDIRPath, 'tmp', 'account.txt'),mode='wb',encoding='utf-8') as f:
            ...
            f.write('%s %s %s'%(self.AppInfo['DicttoolDBName'], self.AppInfo['DicttoolDBUser'], self.AppInfo['DicttoolDBPassword'])+'\n')
