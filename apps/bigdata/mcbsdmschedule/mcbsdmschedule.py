...
class MCBSDMScheduleTool(object):
    def __init__(self, namespace='default', mcbsdmschedulelog='mcb-sdm-schedule-pv-log' , mcbsdmscheduledata='mcb-sdm-schedule-pv-data',
        ...
        self.AppInfo['MCBSDMScheduleLogPath'] = os.path.join(self.AppInfo['LogStorageBasePath'], '-'.join([namespace, mcbsdmschedulelog]))
        self.AppInfo['MCBSDMScheduleDataPath'] = os.path.join(self.AppInfo['DataStorageBasePath'], '-'.join([namespace, mcbsdmscheduledata]))
    ...
    def setupStorage(self):
        ...
        print ('create MCBSDMSchedule Storage successfully')


        self.LogStorageObj.createSubFolder(self.AppInfo['MCBSDMScheduleLogPath'])
        self.DataStorageObj.createSubFolder(self.AppInfo['MCBSDMScheduleDataPath'])

        self.TmpStoragePathDict = dict()
        self.TmpStoragePathDict['MCBSDMScheduleDataPath'] = self.DataStorageObj.generateRealPath(self.AppInfo['MCBSDMScheduleDataPath'])
        self.TmpStoragePathDict['MCBSDMScheduleLogPath'] = self.LogStorageObj.generateRealPath(self.AppInfo['MCBSDMScheduleLogPath'])

        print ('setup MCBSDMSchedule Storage successfully')
    ...
    def generateValues(self):
        self.AppInfo['MCBSDMPushAppImage'] = replaceDockerRepo(self.AppInfo['MCBSDMPushAppImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['MCBSDMWebiteTouTiaoImage'] = replaceDockerRepo(self.AppInfo['MCBSDMWebiteTouTiaoImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['MCBSDMFeifanImage'] = replaceDockerRepo(self.AppInfo['MCBSDMFeifanImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['MCBSDMWeixinImage'] = replaceDockerRepo(self.AppInfo['MCBSDMWeixinImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['MCBSDMSZBOriginalImage'] = replaceDockerRepo(self.AppInfo['MCBSDMSZBOriginalImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['MCBSDMProductImage'] = replaceDockerRepo(self.AppInfo['MCBSDMProductImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['MCBSDMWeiBoYuQingImage'] = replaceDockerRepo(self.AppInfo['MCBSDMWeiBoYuQingImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['MCBSDMNewMediaImage'] = replaceDockerRepo(self.AppInfo['MCBSDMNewMediaImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['MCBSDMTraditionalYuQingImage'] = replaceDockerRepo(self.AppInfo['MCBSDMTraditionalYuQingImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['MCBSDMWeiboImage'] = replaceDockerRepo(self.AppInfo['MCBSDMWeiboImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['MCBSDMWebiteImage'] = replaceDockerRepo(self.AppInfo['MCBSDMWebiteImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['MCBSDMWeixinYuQingImage'] = replaceDockerRepo(self.AppInfo['MCBSDMWeixinYuQingImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['MCBSDMEpaperbmImage'] = replaceDockerRepo(self.AppInfo['MCBSDMEpaperbmImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['MCBSDMMeiziImage'] = replaceDockerRepo(self.AppInfo['MCBSDMMeiziImage'], self.AppInfo['HarborAddr'])


        self.AppInfo['NFSProvisionerImage'] =replaceDockerRepo(self.AppInfo['NFSProvisionerImage'],
                                                               self.AppInfo['HarborAddr'])
        '''if not self.AppInfo['MCBSDMSchedulePrimaryDBPassword']:
            self.AppInfo['MCBSDMSchedulePrimaryDBPassword'] = crypto_tools.generateRandomAlphaNumericString(lenght=10)
        '''
    ...
    def applyYAML(self):
        ...
        print ('Apply MCBSDMSchedule ....')
        if not self.k8sObj.checkNamespacedResourceHealth(name='sdm-push-app', namespace=self.AppInfo['Namespace'],
                                                         kind='Deployment'):
            try:
                self.k8sObj.deleteNamespacedDeployment(name='sdm-push-app', namespace=self.AppInfo['Namespace'])
            except:
                pass
            ...
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'mcb-sdm-schedule-cm.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'mcb-sdm-schedule-svc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/mcb-sdm-schedule-pv.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/mcb-sdm-schedule-pvc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )


            TmpResponse = self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'mcb-sdm-schedule-deploy.yaml'),
                                                         namespace=self.AppInfo['Namespace'])
        ...
        for itime in range(self.RetryTimes):
            TmpResponse = self.k8sObj.getNamespacedDeployment(name='sdm-push-app',
            ...
            TmpCheckResult = TmpServiceCheckObj.checkServicePortState(targetaddress='sdm-website-svc:9017')
            print ('sdm-website-svc:9017 is listening....')
    ...
    def start(self):
        ...
        if TmpResponse['ret_code'] != 0:
            print ('failed to install MCBSDMSchedule')
            return TmpResponse
    ...
    def installDependences(self):
        for TmpRawStr in self.AppInfo['dependences']:
            ...
            if TmpClsName == 'MariaDBTool':
            ...
            elif TmpClsName == 'DicttoolTool':
                self.AppInfo['MCBSDMSchedulePrimaryDBPassword'] = TmpInfo['DicttoolDBPassword']
                ...
                self.AppInfo['MCBSDMScheduleMongoDBPassword'] = crypto_tools.DecodeBase64(TmpInfo['MongoPassword'])
    ...
    def PreInstall(self):
        ...
        with open(os.path.join(self.BaseDIRPath, 'tmp', 'account.txt'),mode='wb',encoding='utf-8') as f:
            ...
            f.write('%s %s %s'%(self.AppInfo['MCBSDMSchedulePrimaryDBName'], self.AppInfo['MCBSDMSchedulePrimaryDBUser'], self.AppInfo['MCBSDMSchedulePrimaryDBPassword'])+'\n')
    ...
    def postInstall(self):
        ...
        '''
        self.DataStorageObj.uploadFile(localpath=os.path.join(self.BaseDIRPath, 'downloads', 'mcbpm.conf'),
...
if __name__ == "__main__":
    tmp = MCBSDMScheduleTool(namespace='sly2', nfsinfo=dict(hostname='192.168.200.74', port=1022, username='root', password='!QAZ2wsx1234',
                         basepath='/TRS/DATA'))
