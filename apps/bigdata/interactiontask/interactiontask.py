...
class InteractionTaskTool(object):
    def __init__(self, namespace='default', interactiontasklog='interaction-task-pv-log' ,
        ...
        self.AppInfo['InteractionTaskLogPath'] = os.path.join(self.AppInfo['LogStorageBasePath'], '-'.join([namespace, interactiontasklog]))
    ...
    def setupStorage(self):
        ...
        print ('create InteractionTask Storage successfully')


        self.LogStorageObj.createSubFolder(self.AppInfo['InteractionTaskLogPath'])


        self.TmpStoragePathDict = dict()
        self.TmpStoragePathDict['InteractionTaskLogPath'] = self.LogStorageObj.generateRealPath(self.AppInfo['InteractionTaskLogPath'])

        print ('setup InteractionTask Storage successfully')
    ...
    def generateValues(self):
        self.AppInfo['InteractionTaskImage'] = replaceDockerRepo(self.AppInfo['InteractionTaskImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['NFSProvisionerImage'] =replaceDockerRepo(self.AppInfo['NFSProvisionerImage'],
                                                               self.AppInfo['HarborAddr'])
        if not self.AppInfo['InteractionTaskDBPassword']:
            self.AppInfo['InteractionTaskDBPassword'] = crypto_tools.generateRandomAlphaNumericString(lenght=10)
    ...
    def applyYAML(self):
        ...
        print ('Apply InteractionTask ....')
        if not self.k8sObj.checkNamespacedResourceHealth(name='interaction-task-deploy', namespace=self.AppInfo['Namespace'],
                                                         kind='Deployment'):
            try:
                self.k8sObj.deleteNamespacedDeployment(name='interaction-task-deploy', namespace=self.AppInfo['Namespace'])
            except:
                pass
            ...
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'interaction-task-cm.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'interaction-task-svc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/interaction-task-pv.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/interaction-task-pvc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )


            TmpResponse = self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'interaction-task-deploy.yaml'),
                                                         namespace=self.AppInfo['Namespace'])
        ...
        for itime in range(self.RetryTimes):
            TmpResponse = self.k8sObj.getNamespacedDeployment(name='interaction-task-deploy',
            ...
            TmpCheckResult = TmpServiceCheckObj.checkServicePortState(targetaddress='interaction-task-svc:9999')
            print ('interaction-task-svc:9999 is listening....')
    ...
    def start(self):
        ...
        if TmpResponse['ret_code'] != 0:
            print ('failed to install InteractionTask')
            return TmpResponse
    ...
    def PreInstall(self):
        ...
        with open(os.path.join(self.BaseDIRPath, 'tmp', 'account.txt'),mode='wb',encoding='utf-8') as f:
            ...
            f.write('%s %s %s'%(self.AppInfo['InteractionTaskDBName'], self.AppInfo['InteractionTaskDBUser'], self.AppInfo['InteractionTaskDBPassword'])+'\n')
    ...
    def postInstall(self):
        ...
        #self.SSHClient.uploadFile(localpath=os.path.join(self.BaseDIRPath, 'downloads', 'dicttool.conf'),
...
if __name__ == "__main__":
    tmp = InteractionTaskTool(namespace='sly2', nfsinfo=dict(hostname='192.168.200.74', port=1022, username='root', password='!QAZ2wsx1234',
                         basepath='/TRS/DATA'))
