...
class TmyDecisionCenterTool(object):
    def __init__(self, namespace='default', tmydecisioncenterlog='tmy-decision-center-pv-log' ,  nfsinfo={},
        ...
        self.AppInfo['TmyDecisionCenterLogPath'] = os.path.join(self.AppInfo['NFSBasePath'], '-'.join([namespace, tmydecisioncenterlog]))
    ...
    def generateValues(self):
        self.AppInfo['TmyDecisionCenterImage'] = replaceDockerRepo(self.AppInfo['TmyDecisionCenterImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['NFSProvisionerImage'] =replaceDockerRepo(self.AppInfo['NFSProvisionerImage'], self.AppInfo['HarborAddr'])

        if not self.AppInfo['TmyDecisionCenterPrimaryDBPassword']:
            self.AppInfo['TmyDecisionCenterPrimaryDBPassword'] = crypto_tools.generateRandomAlphaNumericString(lenght=10)
    ...
    def applyYAML(self):
        ...
        print ('Apply TmyDecisionCenter ....')
        if not self.k8sObj.checkNamespacedResourceHealth(name='tmy-decision-center-deploy', namespace=self.AppInfo['Namespace'],
                                                         kind='Deployment'):
            try:
                self.k8sObj.deleteNamespacedDeployment(name='tmy-decision-center-deploy', namespace=self.AppInfo['Namespace'])
            except:
                pass
            ...
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'tmy-decision-center-cm.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'tmy-decision-center-svc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/tmy-decision-center-pv.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/tmy-decision-center-pvc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )


            TmpResponse = self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'tmy-decision-center-deploy.yaml'),
                                                         namespace=self.AppInfo['Namespace'])
        ...
        for itime in range(self.RetryTimes):
            TmpResponse = self.k8sObj.getNamespacedDeployment(name='tmy-decision-center-deploy',
            ...
            TmpCheckResult = TmpServiceCheckObj.checkServicePortState(targetaddress='tmy-decision-center-svc:58223')
            print ('tmy-decision-center-svc:58223 is listening....')
    ...
    def start(self):
        ...
        if TmpResponse['ret_code'] != 0:
            print ('failed to install TmyDecisionCenter')
            return TmpResponse
    ...
    def PreInstall(self):
        ...
        with open(os.path.join(self.BaseDIRPath, 'tmp', 'account.txt'),mode='wb',encoding='utf-8') as f:
            ...
            f.write('%s %s %s'%(self.AppInfo['TmyDecisionCenterPrimaryDBName'], self.AppInfo['TmyDecisionCenterPrimaryDBUser'], self.AppInfo['TmyDecisionCenterPrimaryDBPassword'])+'\n')
    ...
    def postInstall(self):
        ...
        '''
        self.SSHClient.uploadFile(localpath=os.path.join(self.BaseDIRPath, 'downloads', 'tmydecisioncenter.conf'),
                                  remotepath=os.path.join(TmpNginxConfigPath, 'tmydecisioncenter.conf')
                                  )
        '''
...
if __name__ == "__main__":
    tmp = TmyDecisionCenterTool(namespace='sly2', nfsinfo=dict(hostname='192.168.200.74', port=1022, username='root', password='!QAZ2wsx1234',
                         basepath='/TRS/DATA'))
