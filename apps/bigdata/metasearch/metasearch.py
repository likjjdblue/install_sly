...
class MetaSearchTool(object):
    def __init__(self, namespace='default', metasearchlog='metasearch-pv-log' ,
        ...
        self.AppInfo['MetaSearchLogPath'] = os.path.join(self.AppInfo['LogStorageBasePath'], '-'.join([namespace, metasearchlog]))
    ...
    def setupStorage(self):
        ...
        print ('create MetaSearch Storage successfully')


        self.LogStorageObj.createSubFolder(self.AppInfo['MetaSearchLogPath'])

        self.TmpStoragePathDict = dict()
        self.TmpStoragePathDict['MetaSearchLogPath'] = self.LogStorageObj.generateRealPath(self.AppInfo['MetaSearchLogPath'])


        print ('setup MetaSearch Storage successfully')
    ...
    def applyYAML(self):
        ...
        print ('Apply MetaSearch ....')
        if not self.k8sObj.checkNamespacedResourceHealth(name='metasearch-deploy', namespace=self.AppInfo['Namespace'],
                                                         kind='Deployment'):
            try:
                self.k8sObj.deleteNamespacedDeployment(name='metasearch-deploy', namespace=self.AppInfo['Namespace'])
            except:
                pass
            ...
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'metasearch-cm.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'metasearch-svc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            ...
            TmpResponse = self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'metasearch-deploy.yaml'),
                                                         namespace=self.AppInfo['Namespace'])
        ...
        for itime in range(self.RetryTimes):
            TmpResponse = self.k8sObj.getNamespacedDeployment(name='metasearch-deploy',
            ...
            TmpCheckResult = TmpServiceCheckObj.checkServicePortState(targetaddress='metasearch-svc:8650')
            print ('metasearch-svc:8650 is listening....')
    ...
    def start(self):
        ...
        if TmpResponse['ret_code'] != 0:
            print ('failed to install MetaSearch')
            return TmpResponse
    ...
    def PreInstall(self):
        ...
        with open(os.path.join(self.BaseDIRPath, 'tmp', 'account.txt'),mode='wb',encoding='utf-8') as f:
            ...
            f.write('%s %s %s'%(self.AppInfo['MetaSearchPrimaryDBName'], self.AppInfo['MetaSearchPrimaryDBUser'], self.AppInfo['MetaSearchPrimaryDBPassword'])+'\n')
    ...
    def postInstall(self):
        ...
        self.DataStorageObj.uploadFile(localpath=os.path.join(self.BaseDIRPath, 'downloads', 'metasearch.conf'),
...
if __name__ == "__main__":
    tmp = MetaSearchTool(namespace='sly2', nfsinfo=dict(hostname='192.168.200.74', port=1022, username='root', password='!QAZ2wsx1234',
                         basepath='/TRS/DATA'))
