...
class TimingSchedulerTool(object):
    def __init__(self, namespace='default', timingschedulerlog='timing-scheduler-pv-log' ,
        ...
        self.AppInfo['TimingSchedulerLogPath'] = os.path.join(self.AppInfo['LogStorageBasePath'], '-'.join([namespace, timingschedulerlog]))
    ...
    def setupStorage(self):
        ...
        print ('create TimingScheduler Storage successfully')


        self.LogStorageObj.createSubFolder(self.AppInfo['TimingSchedulerLogPath'])

        self.TmpStoragePathDict = dict()
        self.TmpStoragePathDict['TimingSchedulerLogPath'] = self.LogStorageObj.generateRealPath(self.AppInfo['TimingSchedulerLogPath'])

        print ('setup TimingScheduler Storage successfully')
    ...
    def generateValues(self):
        self.AppInfo['TimingSchedulerImage'] = replaceDockerRepo(self.AppInfo['TimingSchedulerImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['NFSProvisionerImage'] =replaceDockerRepo(self.AppInfo['NFSProvisionerImage'], self.AppInfo['HarborAddr'])

        if not self.AppInfo['TimingSchedulerPrimaryDBPassword']:
            self.AppInfo['TimingSchedulerPrimaryDBPassword'] = crypto_tools.generateRandomAlphaNumericString(lenght=10)
    ...
    def applyYAML(self):
        ...
        print ('Apply TimingScheduler ....')
        if not self.k8sObj.checkNamespacedResourceHealth(name='timing-scheduler-deploy', namespace=self.AppInfo['Namespace'],
                                                         kind='Deployment'):
            try:
                self.k8sObj.deleteNamespacedDeployment(name='timing-scheduler-deploy', namespace=self.AppInfo['Namespace'])
            except:
                pass
            ...
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'timing_scheduler-cm.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'timing_scheduler-svc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/timing_scheduler-pv.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/timing_scheduler-pvc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )


            TmpResponse = self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'timing_scheduler-deploy.yaml'),
                                                         namespace=self.AppInfo['Namespace'])
        ...
        for itime in range(self.RetryTimes):
            TmpResponse = self.k8sObj.getNamespacedDeployment(name='timing-scheduler-deploy',
            ...
            TmpCheckResult = TmpServiceCheckObj.checkServicePortState(targetaddress='timing-scheduler-svc:9009')
            print ('timing-scheduler-svc:9009 is listening....')
    ...
    def start(self):
        ...
        if TmpResponse['ret_code'] != 0:
            print ('failed to install TimingScheduler')
            return TmpResponse
    ...
    def PreInstall(self):
        ...
        with open(os.path.join(self.BaseDIRPath, 'tmp', 'account.txt'),mode='wb',encoding='utf-8') as f:
            ...
            f.write('%s %s %s'%(self.AppInfo['TimingSchedulerPrimaryDBName'], self.AppInfo['TimingSchedulerPrimaryDBUser'], self.AppInfo['TimingSchedulerPrimaryDBPassword'])+'\n')
        ...
        print (os.path.join(self.BaseDIRPath, 'downloads', 'timing.sql'))
        ...
        self.DataStorageObj.uploadFile(localpath=os.path.join(self.BaseDIRPath, 'downloads', 'timing.sql'),
                                  remotepath=os.path.join(TmpSQLToolSQLPath, 'timing.sql')
                                  )
    ...
    def postInstall(self):
        ...
        self.DataStorageObj.uploadFile(localpath=os.path.join(self.BaseDIRPath, 'downloads', 'timingscheduler.conf'),
                                  remotepath=os.path.join(TmpNginxConfigPath, 'timingscheduler.conf')
                                  )
...
if __name__ == "__main__":
    tmp = TimingSchedulerTool(namespace='sly2', nfsinfo=dict(hostname='192.168.200.74', port=1022, username='root', password='!QAZ2wsx1234',
                         basepath='/TRS/DATA'))
