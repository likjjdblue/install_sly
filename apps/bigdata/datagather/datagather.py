...
class DataGatherTool(object):
    def __init__(self, namespace='default', datagatherlog='gather-consumer-pv-log' , datagatherdata='gather-consumer-pv-data' ,
        ...
        self.AppInfo['DataGatherLogPath'] = os.path.join(self.AppInfo['NFSBasePath'], '-'.join([namespace, datagatherlog]))
        self.AppInfo['DataGatherDataPath'] = os.path.join(self.AppInfo['NFSBasePath'], '-'.join([namespace, datagatherdata]))
    ...
    def generateValues(self):
        self.AppInfo['DataGatherImage'] = replaceDockerRepo(self.AppInfo['DataGatherImage'], self.AppInfo['HarborAddr'])
    ...
    def applyYAML(self):
        ...
        print ('Apply DataGather ....')
        if not self.k8sObj.checkNamespacedResourceHealth(name='gather-consumer-deploy', namespace=self.AppInfo['Namespace'],
                                                         kind='Deployment'):
            try:
                self.k8sObj.deleteNamespacedDeployment(name='gather-consumer-deploy', namespace=self.AppInfo['Namespace'])
            except:
                pass
            ...
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'gather-consumer-cm.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'gather-consumer-svc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/gather-consumer-pv.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/gather-consumer-pvc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )


            TmpResponse = self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'gather-consumer-deploy.yaml'),
                                                         namespace=self.AppInfo['Namespace'])
        ...
        for itime in range(self.RetryTimes):
            TmpResponse = self.k8sObj.getNamespacedDeployment(name='gather-consumer-deploy',
    ...
    def start(self):
        ...
        if TmpResponse['ret_code'] != 0:
            print ('failed to install DataGather')
            return TmpResponse
    ...
    def PreInstall(self):
        ...
        with open(os.path.join(self.BaseDIRPath, 'tmp', 'account.txt'),mode='wb',encoding='utf-8') as f:
            ...
            f.write('%s %s %s'%(self.AppInfo['BigdataAccessDBName'], self.AppInfo['BigdataAccessDBUser'], self.AppInfo['BigdataAccessDBPassword'])+'\n')
...
if __name__ == "__main__":
    tmp = DataGatherTool(namespace='sly2', nfsinfo=dict(hostname='192.168.200.74', port=1022, username='root', password='!QAZ2wsx1234',
                         basepath='/TRS/DATA'))
