...
class InteractionServiceCenterTool(object):
    ...
    def setupStorage(self):
        ...
        self.TmpStoragePathDict['InteractionServiceCenterLogPath'] = self.LogStorageObj.generateRealPath(self.AppInfo['InteractionServiceCenterLogPath'])
    ...
    def generateValues(self):
        self.AppInfo['InteractionServiceCenterImage'] = replaceDockerRepo(self.AppInfo['InteractionServiceCenterImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['NFSProvisionerImage'] =replaceDockerRepo(self.AppInfo['NFSProvisionerImage'],

                                                               self.AppInfo['HarborAddr'])

        '''
        if not self.AppInfo['InteractionServiceCenterDBPassword']:
            self.AppInfo['InteractionServiceCenterDBPassword'] = crypto_tools.generateRandomAlphaNumericString(lenght=10)
        '''
    ...
    def applyYAML(self):
        ...
        print ('Apply InteractionServiceCenter ....')
        if not self.k8sObj.checkNamespacedResourceHealth(name='interaction-service-center-deploy', namespace=self.AppInfo['Namespace'],
                                                         kind='Deployment'):
            try:
                self.k8sObj.deleteNamespacedDeployment(name='interaction-service-center-deploy', namespace=self.AppInfo['Namespace'])
            except:
                pass
            ...
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'interaction-service-center-cm.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'interaction-service-center-svc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/interaction-service-center-pv.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/interaction-service-center-pvc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )


            TmpResponse = self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'interaction-service-center-deploy.yaml'),
                                                         namespace=self.AppInfo['Namespace'])
        ...
        for itime in range(self.RetryTimes):
            TmpResponse = self.k8sObj.getNamespacedDeployment(name='interaction-service-center-deploy',
            ...
            TmpCheckResult = TmpServiceCheckObj.checkServicePortState(targetaddress='interaction-service-center-svc:9998')
            print ('interaction-service-center-svc:9998 is listening....')
    ...
    def start(self):
        ...
        if TmpResponse['ret_code'] != 0:
            print ('failed to install InteractionServiceCenter')
            return TmpResponse
    ...
    def installDependences(self):
        for TmpRawStr in self.AppInfo['dependences']:
            ...
            elif TmpClsName == 'InteractionTaskTool':
                self.AppInfo['InteractionServiceCenterDBPassword'] = TmpInfo['InteractionTaskDBPassword']
...
if __name__ == "__main__":
    tmp = InteractionServiceCenterTool(namespace='sly2', nfsinfo=dict(hostname='192.168.200.74', port=1022, username='root', password='!QAZ2wsx1234',
                         basepath='/TRS/DATA'))
