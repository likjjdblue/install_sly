...
class PictureCenterTool(object):
    def __init__(self, namespace='default', picturecenterlog='picturecenter-pv-log' , picturecenterdata='picturecenter-pv-data',
        ...
        self.AppInfo['PictureCenterLogPath'] = os.path.join(self.AppInfo['LogStorageBasePath'], '-'.join([namespace, picturecenterlog]))
        self.AppInfo['PictureCenterDataPath'] = os.path.join(self.AppInfo['DataStorageBasePath'], '-'.join([namespace, picturecenterdata]))
    ...
    def setupStorage(self):
        ...
        print ('create PictureCenter Storage successfully')


        self.LogStorageObj.createSubFolder(self.AppInfo['PictureCenterLogPath'])
        self.DataStorageObj.createSubFolder(self.AppInfo['PictureCenterDataPath'])
    ...
    def applyYAML(self):
        ...
        print ('Apply PictureCenter ....')
        if not self.k8sObj.checkNamespacedResourceHealth(name='picturecenter-deploy', namespace=self.AppInfo['Namespace'],
                                                         kind='Deployment'):
            try:
                self.k8sObj.deleteNamespacedDeployment(name='picturecenter-deploy', namespace=self.AppInfo['Namespace'])
            except:
                pass
            ...
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'picturecenter-cm.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'picturecenter-svc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/picturecenter-pv.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/picturecenter-pvc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )


            TmpResponse = self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'picturecenter-deploy.yaml'),
                                                         namespace=self.AppInfo['Namespace'])
        ...
        for itime in range(self.RetryTimes):
            TmpResponse = self.k8sObj.getNamespacedDeployment(name='picturecenter-deploy',
            ...
            TmpCheckResult = TmpServiceCheckObj.checkServicePortState(targetaddress='picturecenter-svc:58400')
            print ('picturecenter-svc:58400 is listening....')
    ...
    def start(self):
        ...
        if TmpResponse['ret_code'] != 0:
            print ('failed to install PictureCenter')
            return TmpResponse
    ...
    def PreInstall(self):
        ...
        with open(os.path.join(self.BaseDIRPath, 'tmp', 'account.txt'),mode='wb',encoding='utf-8') as f:
            ...
            f.write('%s %s %s'%(self.AppInfo['PictureCenterPrimaryDBName'], self.AppInfo['PictureCenterPrimaryDBUser'], self.AppInfo['PictureCenterPrimaryDBPassword'])+'\n')
...
if __name__ == "__main__":
    tmp = PictureCenterTool(namespace='sly2', nfsinfo=dict(hostname='192.168.200.74', port=1022, username='root', password='!QAZ2wsx1234',
                         basepath='/TRS/DATA'))
