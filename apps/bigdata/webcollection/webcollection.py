...
class WebCollectionTool(object):
    def __init__(self, namespace='default', webcollectionlog='webcollection-pv-log',
        ...
        self.AppInfo['WebCollectionLogPath'] = os.path.join(self.AppInfo['LogStorageBasePath'], '-'.join([namespace, webcollectionlog]))
    ...
    def setupStorage(self):
        ...
        print ('create WebCollection Storage successfully')


        self.LogStorageObj.createSubFolder(self.AppInfo['WebCollectionLogPath'])

        self.TmpStoragePathDict = dict()
        self.TmpStoragePathDict['WebCollectionLogPath'] = self.LogStorageObj.generateRealPath(self.AppInfo['WebCollectionLogPath'])

        print ('setup WebCollection Storage successfully')
    ...
    def generateValues(self):
        self.AppInfo['WebCollectionImage'] = replaceDockerRepo(self.AppInfo['WebCollectionImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['NFSProvisionerImage'] =replaceDockerRepo(self.AppInfo['NFSProvisionerImage'],
                                                               self.AppInfo['HarborAddr'])
        if not self.AppInfo['WebCollectionPrimaryDBPassword']:
            self.AppInfo['WebCollectionPrimaryDBPassword'] = crypto_tools.generateRandomAlphaNumericString(lenght=10)
    ...
    def applyYAML(self):
        ...
        print ('Apply WebCollection ....')
        if not self.k8sObj.checkNamespacedResourceHealth(name='webcollection-deploy', namespace=self.AppInfo['Namespace'],
                                                         kind='Deployment'):
            try:
                self.k8sObj.deleteNamespacedDeployment(name='webcollection-deploy', namespace=self.AppInfo['Namespace'])
            except:
                pass
            ...
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'webcollection-cm.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'webcollection-svc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/webcollection-pv.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/webcollection-pvc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )


            TmpResponse = self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'webcollection-deploy.yaml'),
                                                         namespace=self.AppInfo['Namespace'])
        ...
        for itime in range(self.RetryTimes):
            TmpResponse = self.k8sObj.getNamespacedDeployment(name='webcollection-deploy',
            ...
            TmpCheckResult = TmpServiceCheckObj.checkServicePortState(targetaddress='webcollection-svc:7176')
            print ('webcollection-svc:7176 is listening....')
    ...
    def start(self):
        ...
        if TmpResponse['ret_code'] != 0:
            print ('failed to install WebCollection')
            return TmpResponse
    ...
    def installDependences(self):
        for TmpRawStr in self.AppInfo['dependences']:
            ...
            if TmpClsName == 'MariaDBTool':
                ...
                self.AppInfo['WebCollectionSecondDBPassword'] = TmpInfo['DicttoolDBPassword']
    ...
    def PreInstall(self):
        ...
        with open(os.path.join(self.BaseDIRPath, 'tmp', 'account.txt'),mode='wb',encoding='utf-8') as f:
            ...
            f.write('%s %s %s'%(self.AppInfo['WebCollectionPrimaryDBName'], self.AppInfo['WebCollectionPrimaryDBUser'], self.AppInfo['WebCollectionPrimaryDBPassword'])+'\n')
        ...
        print (os.path.join(self.BaseDIRPath, 'downloads', 'websitecollection.sql'))
        ...
        self.DataStorageObj.uploadFile(localpath=os.path.join(self.BaseDIRPath, 'downloads', 'websitecollection.sql'),
                                  remotepath=os.path.join(TmpSQLToolSQLPath, 'websitecollection.sql')
                                  )
    ...
    def postInstall(self):
        ...
        self.DataStorageObj.uploadFile(localpath=os.path.join(self.BaseDIRPath, 'downloads', 'webcollection.conf'),
                                  remotepath=os.path.join(TmpNginxConfigPath, 'webcollection.conf')
                                  )
...
if __name__ == "__main__":
    tmp = WebCollectionTool(namespace='sly2', nfsinfo=dict(hostname='192.168.200.74', port=1022, username='root', password='!QAZ2wsx1234',
                         basepath='/TRS/DATA'))
