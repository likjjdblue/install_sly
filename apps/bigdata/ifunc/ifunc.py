...
class iFuncTool(object):
    def __init__(self, namespace='default', ifunclog='ifunc-pv-log' ,
        ...
        self.AppInfo['iFuncLogPath'] = os.path.join(self.AppInfo['LogStorageBasePath'], '-'.join([namespace, ifunclog]))
    ...
    def setupStorage(self):
        ...
        print ('create iFunc Storage successfully')


        self.LogStorageObj.createSubFolder(self.AppInfo['iFuncLogPath'])


        self.TmpStoragePathDict = dict()
        self.TmpStoragePathDict['iFuncLogPath'] = self.LogStorageObj.generateRealPath(self.AppInfo['iFuncLogPath'])

        print ('setup iFunc Storage successfully')
    ...
    def generateValues(self):
        self.AppInfo['iFuncImage'] = replaceDockerRepo(self.AppInfo['iFuncImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['NFSProvisionerImage'] =replaceDockerRepo(self.AppInfo['NFSProvisionerImage'],
                                                               self.AppInfo['HarborAddr'])
        '''
        if not self.AppInfo['iFuncPrimaryDBPassword']:
            self.AppInfo['iFuncPrimaryDBPassword'] = crypto_tools.generateRandomAlphaNumericString(lenght=10)
        '''
    ...
    def applyYAML(self):
        ...
        print ('Apply iFunc ....')
        if not self.k8sObj.checkNamespacedResourceHealth(name='transfer-ifuncun-deploy', namespace=self.AppInfo['Namespace'],
                                                         kind='Deployment'):
            try:
                self.k8sObj.deleteNamespacedDeployment(name='transfer-ifuncun-deploy', namespace=self.AppInfo['Namespace'])
            except:
                pass
            ...
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'transfer_ifuncun-cm.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'transfer_ifuncun-svc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            '''
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/metasearch-pv.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/metasearch-pvc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            '''


            TmpResponse = self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'transfer_ifuncun-deploy.yaml'),
                                                         namespace=self.AppInfo['Namespace'])
        ...
        for itime in range(self.RetryTimes):
            TmpResponse = self.k8sObj.getNamespacedDeployment(name='transfer-ifuncun-deploy',
            ...
            TmpCheckResult = TmpServiceCheckObj.checkServicePortState(targetaddress='transfer-ifuncun-svc:8881')
            print ('transfer-ifuncun-svc:8881 is listening....')
    ...
    def PreInstall(self):
        ...
        with open(os.path.join(self.BaseDIRPath, 'tmp', 'account.txt'),mode='wb',encoding='utf-8') as f:
            ...
            f.write('%s %s %s'%(self.AppInfo['iFuncPrimaryDBName'], self.AppInfo['iFuncPrimaryDBUser'], self.AppInfo['iFuncPrimaryDBPassword'])+'\n')
    ...
    def postInstall(self):
        ...
        self.DataStorageObj.uploadFile(localpath=os.path.join(self.BaseDIRPath, 'downloads', 'ifunc.conf'),
...
if __name__ == "__main__":
    tmp = iFuncTool(namespace='sly2', nfsinfo=dict(hostname='192.168.200.74', port=1022, username='root', password='!QAZ2wsx1234',
                         basepath='/TRS/DATA'))
