...
class MCNAdminTool(object):
    def __init__(self, namespace='default', mcnadmindatapath='mcn-pv-data/MCNData', mcnadminlogdata='mcnadmin-pv-log' ,
        ...
        self.AppInfo['MCNAdminDataPath'] = os.path.join(self.AppInfo['DataStorageBasePath'], '-'.join([namespace, TmpmcndatapathList[0]]), TmpmcndatapathList[1])
        self.AppInfo['MCNAdminLogPath'] = os.path.join(self.AppInfo['LogStorageBasePath'], '-'.join([namespace, mcnadminlogdata]))
    ...
    def setupStorage(self):
        ...
        print ('create MCN Admin Storage successfully')

        self.DataStorageObj.createSubFolder(self.AppInfo['MCNAdminDataPath'])
        self.LogStorageObj.createSubFolder(self.AppInfo['MCNAdminLogPath'])

        self.TmpStoragePathDict = dict()
        self.TmpStoragePathDict['MCNAdminDataPath'] = self.DataStorageObj.generateRealPath(self.AppInfo['MCNAdminDataPath'])
        self.TmpStoragePathDict['MCNAdminLogPath'] = self.LogStorageObj.generateRealPath(self.AppInfo['MCNAdminLogPath'])


        print ('setup MCN Admin Storage successfully')
    ...
    def generateValues(self):
        ...
        if not self.AppInfo['UPC_QuartzDBPassword']:
            self.AppInfo['UPC_QuartzDBPassword'] = crypto_tools.generateRandomAlphaNumericString(lenght=10)

        if not self.AppInfo['UPC_MrsDBPassword']:
            self.AppInfo['UPC_MrsDBPassword'] = crypto_tools.generateRandomAlphaNumericString(lenght=10)
        '''
    ...
    def applyYAML(self):
        ...
        for itime in range(self.RetryTimes):
            ...
            TmpCheckResult = TmpServiceCheckObj.checkServicePortState(targetaddress='mcnadmin-svc:9002')
            print ('mcnadmin-svc:9002 is listening....')
