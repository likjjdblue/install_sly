...
class NewMediaPlatformTool(object):
    def __init__(self, namespace='default', newmediaplatformlogpath='newmediaplatform-pv-log', newmediaplatformdatapath='newmediaplatform-pv-data',
        ...
        self.AppInfo['NewMediaPlatformLogPath'] = os.path.join(self.AppInfo['NFSBasePath'], '-'.join([namespace, newmediaplatformlogpath]))
        self.AppInfo['NewMediaPlatformDataPath'] = os.path.join(self.AppInfo['NFSBasePath'], '-'.join([namespace, newmediaplatformdatapath]))
    ...
    def setupNFS(self):
        ...
        print ('create NewMediaPlatform NFS successfully')

        self.NFSObj.createSubFolder(self.AppInfo['NewMediaPlatformLogPath'])
        self.NFSObj.createSubFolder(self.AppInfo['NewMediaPlatformDataPath'])

        print ('setup NewMediaPlatform NFS successfully')
    ...
    def generateValues(self):
        self.AppInfo['NewMediaPlatformImage'] = replaceDockerRepo(self.AppInfo['NewMediaPlatformImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['NFSProvisionerImage'] =replaceDockerRepo(self.AppInfo['NFSProvisionerImage'],
                                                               self.AppInfo['HarborAddr'])
        if not self.AppInfo['NewMediaPlatformDBPassword']:
            self.AppInfo['NewMediaPlatformDBPassword'] = crypto_tools.generateRandomAlphaNumericString(lenght=10)
    ...
    def applyYAML(self):
        ...
        print ('Apply TRS NewMediaPlatform ....')
        if not self.k8sObj.checkNamespacedResourceHealth(name='newmediaplatform-deploy', namespace=self.AppInfo['Namespace'],
                                                         kind='Deployment'):
            try:
                self.k8sObj.deleteNamespacedDeployment(name='newmediaplatform-deploy', namespace=self.AppInfo['Namespace'])
            except:
                pass
            ...
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'newmediaplatform-cm.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'newmediaplatform-svc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/newmediaplatform-pv.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/newmediaplatform-pvc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )


            TmpResponse = self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'newmediaplatform-deploy.yaml'),
                                                         namespace=self.AppInfo['Namespace'])
        ...
        for itime in range(self.RetryTimes):
            TmpResponse = self.k8sObj.getNamespacedDeployment(name='newmediaplatform-deploy',
            ...
            TmpCheckResult = TmpServiceCheckObj.checkServicePortState(targetaddress='newmediaplatform-svc:9012')
            print ('newmediaplatform-svc:9012 is listening....')
    ...
    def start(self):
        ...
        if TmpResponse['ret_code'] != 0:
            print ('failed to install TRS NewMediaPlatform')
            return TmpResponse
    ...
    def PreInstall(self):
        ...
        with open(os.path.join(self.BaseDIRPath, 'tmp', 'account.txt'),mode='wb',encoding='utf-8') as f:
            ...
            f.write('%s %s %s'%(self.AppInfo['NewMediaPlatformDBName'], self.AppInfo['NewMediaPlatformDBUser'], self.AppInfo['NewMediaPlatformDBPassword'])+'\n')
...
if __name__ == "__main__":
    tmp = NewMediaPlatformTool(namespace='sly2', nfsinfo=dict(hostname='192.168.200.74', port=1022, username='root', password='!QAZ2wsx1234',
                         basepath='/TRS/DATA'))
