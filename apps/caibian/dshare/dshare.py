...
class DshareTool(object):
    def __init__(self, namespace='default', downimglogdata='dshare-pv-log' ,
        ...
        self.AppInfo['DshareLogPath'] = os.path.join(self.AppInfo['LogStorageBasePath'], '-'.join([namespace, downimglogdata]))
    ...
    def setupStorage(self):
        ...
        print ('create Dshare Storage successfully')

        self.LogStorageObj.createSubFolder(self.AppInfo['DshareLogPath'])

        self.TmpStoragePathDict = dict()
        self.TmpStoragePathDict['DshareLogPath'] = self.LogStorageObj.generateRealPath(self.AppInfo['DshareLogPath'])

        print ('setup Dshare Storage successfully')
    ...
    def generateValues(self):
        self.AppInfo['DshareImage'] = replaceDockerRepo(self.AppInfo['DshareImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['NFSProvisionerImage'] =replaceDockerRepo(self.AppInfo['NFSProvisionerImage'],
                                                               self.AppInfo['HarborAddr'])
        if not self.AppInfo['DshareDBPassword']:
            self.AppInfo['DshareDBPassword'] = crypto_tools.generateRandomAlphaNumericString(lenght=10)
    ...
    def applyYAML(self):
        ...
        print ('Apply TRS Dshare ....')
        if not self.k8sObj.checkNamespacedResourceHealth(name='dshare-deploy', namespace=self.AppInfo['Namespace'],
                                                         kind='Deployment'):
            try:
                self.k8sObj.deleteNamespacedDeployment(name='dshare-deploy', namespace=self.AppInfo['Namespace'])
            except:
                pass
            ...
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'dshare-cm.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'dshare-svc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/dshare-pv.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/dshare-pvc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )


            TmpResponse = self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'dshare-deploy.yaml'),
                                                         namespace=self.AppInfo['Namespace'])
        ...
        for itime in range(self.RetryTimes):
            TmpResponse = self.k8sObj.getNamespacedDeployment(name='dshare-deploy',
            ...
            TmpCheckResult = TmpServiceCheckObj.checkServicePortState(targetaddress='dshare-svc:9014')
            print ('dshare-svc:9014 is listening....')
    ...
    def start(self):
        ...
        if TmpResponse['ret_code'] != 0:
            print ('failed to install TRS Dshare')
            return TmpResponse
    ...
    def PreInstall(self):
        ...
        with open(os.path.join(self.BaseDIRPath, 'tmp', 'account.txt'),mode='wb',encoding='utf-8') as f:
            ...
            f.write('%s %s %s'%(self.AppInfo['DshareDBName'], self.AppInfo['DshareDBUser'], self.AppInfo['DshareDBPassword'])+'\n')
    ...
    def postInstall(self):
        ...
        self.DataStorageObj.uploadFile(localpath=os.path.join(self.BaseDIRPath, 'downloads', 'dshare.conf'),
...
if __name__ == "__main__":
    tmp = DshareTool(namespace='sly2', nfsinfo=dict(hostname='192.168.0.68', port=22, username='root', password='!QAZ2wsx1234',
                         basepath='/TRS/DATA'))
