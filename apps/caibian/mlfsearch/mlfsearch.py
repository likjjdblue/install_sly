...
class MLFSearchTool(object):
    def __init__(self, namespace='default', mlfsearchlogdata='mlfsearch-pv-log',
        ...
        self.AppInfo['MLFSearchLogPath'] = os.path.join(self.AppInfo['LogStorageBasePath'], '-'.join([namespace, mlfsearchlogdata]))
    ...
    def setupStorage(self):
        ...
        print ('create MLFSearch Storage successfully')

        self.LogStorageObj.createSubFolder(self.AppInfo['MLFSearchLogPath'])


        self.TmpStoragePathDict = dict()
        self.TmpStoragePathDict['MLFSearchLogPath'] = self.LogStorageObj.generateRealPath(self.AppInfo['MLFSearchLogPath'])

        print ('setup MLFSearch Storage successfully')
    ...
    def generateValues(self):
        self.AppInfo['MLFSearchImage'] = replaceDockerRepo(self.AppInfo['MLFSearchImage'], self.AppInfo['HarborAddr'])
    ...
    def applyYAML(self):
        ...
        print ('Apply TRS MLFSearch ....')
        if not self.k8sObj.checkNamespacedResourceHealth(name='mlfsearch-deploy', namespace=self.AppInfo['Namespace'],
                                                         kind='Deployment'):
            try:
                self.k8sObj.deleteNamespacedDeployment(name='mlfsearch-deploy', namespace=self.AppInfo['Namespace'])
            except:
                pass
            ...
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'mlfsearch-cm.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'mlfsearch-svc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/mlfsearch-pv.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'pv/mlfsearch-pvc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )


            TmpResponse = self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'mlfsearch-deploy.yaml'),
                                                         namespace=self.AppInfo['Namespace'])
        ...
        for itime in range(self.RetryTimes):
            TmpResponse = self.k8sObj.getNamespacedDeployment(name='mlfsearch-deploy',
            ...
            TmpCheckResult = TmpServiceCheckObj.checkServicePortState(targetaddress='mlfsearch-svc:9009')
            print ('mlfsearch-svc:9009 is listening....')
    ...
    def start(self):
        ...
        if TmpResponse['ret_code'] != 0:
            print ('failed to install TRS MLFSearch')
            return TmpResponse
    ...
    def postInstall(self):
        ...
        self.DataStorageObj.uploadFile(localpath=os.path.join(self.BaseDIRPath, 'downloads', 'mlfsearch.conf'),
...
if __name__ == "__main__":
    tmp = MLFSearchTool(namespace='sly2', nfsinfo=dict(hostname='192.168.0.68', port=22, username='root', password='!QAZ2wsx1234',
                         basepath='/TRS/DATA'))
