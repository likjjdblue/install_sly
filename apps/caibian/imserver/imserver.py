...
class IMServerTool(object):
    def __init__(self, namespace='default', imserverlogpath='im-server-pv-log',
        ...
        self.AppInfo['IMServerLogPath'] = os.path.join(self.AppInfo['LogStorageBasePath'], '-'.join([namespace, imserverlogpath]))
    ...
    def setupStorage(self):
        ...
        print ('create IMServer Storage successfully')

        self.LogStorageObj.createSubFolder(self.AppInfo['IMServerLogPath'])

        self.TmpStoragePathDict = dict()
        self.TmpStoragePathDict['IMServerLogPath'] = self.LogStorageObj.generateRealPath(self.AppInfo['IMServerLogPath'])

        print ('setup IMServer Storage successfully')
    ...
    def applyYAML(self):
        ...
        for itime in range(self.RetryTimes):
            ...
            TmpCheckResult = TmpServiceCheckObj.checkServicePortState(targetaddress='im-server-svc:8989')
            print ('im-server-svc:8989 is listening....')
    ...
    def postInstall(self):
        ...
        self.DataStorageObj.uploadFile(localpath=os.path.join(self.BaseDIRPath, 'downloads', 'instantmessageservice.conf'),
