...
class CallbackTool(object):
    ...
    def setupStorage(self):
        ...
        self.TmpStoragePathDict['CallbackLogPath'] = self.LogStorageObj.generateRealPath(self.AppInfo['CallbackLogPath'])
    ...
    def generateValues(self):
        self.AppInfo['CallbackImage'] = replaceDockerRepo(self.AppInfo['CallbackImage'], self.AppInfo['HarborAddr'])
        self.AppInfo['NFSProvisionerImage'] =replaceDockerRepo(self.AppInfo['NFSProvisionerImage'],
                                                               self.AppInfo['HarborAddr'])
        '''
        if not self.AppInfo['CallbackPrimaryDBPassword']:
            self.AppInfo['CallbackPrimaryDBPassword'] = crypto_tools.generateRandomAlphaNumericString(lenght=10)
        '''
    ...
    def applyYAML(self):
        ...
        if not self.k8sObj.checkNamespacedResourceHealth(name='callback-deploy', namespace=self.AppInfo['Namespace'],
                                                         kind='Deployment'):
            try:
                self.k8sObj.deleteNamespacedDeployment(name='callback-deploy', namespace=self.AppInfo['Namespace'])
            except:
                pass
            ...
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'callback-cm.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'callback-svc.yaml'),
                                               namespace=self.AppInfo['Namespace']
                                               )
            '''
            ...
            TmpResponse = self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'callback-deploy.yaml'),
                                                         namespace=self.AppInfo['Namespace'])
        ...
        for itime in range(self.RetryTimes):
            TmpResponse = self.k8sObj.getNamespacedDeployment(name='callback-deploy',
            ...
            TmpCheckResult = TmpServiceCheckObj.checkServicePortState(targetaddress='callback-svc:8080')
            print ('callback-svc:8080 is listening....')
    ...
    def start(self):
        ...
        if TmpResponse['ret_code'] != 0:
            print ('failed to install Callback')
            return TmpResponse
...
if __name__ == "__main__":
    tmp = CallbackTool(namespace='sly2', nfsinfo=dict(hostname='192.168.200.74', port=1022, username='root', password='!QAZ2wsx1234',
                         basepath='/TRS/DATA'))
