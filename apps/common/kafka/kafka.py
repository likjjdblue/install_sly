...
class KafkaTool(object):
    ...
    def __init__(self, namespace='default', kafkadatapath='nfs-provisioner',harbor=None, retrytimes=60):
        ...
        self.AppInfo['KafkaDataPath'] = os.path.join(self.AppInfo['DataStorageBasePath'], '-'.join([namespace, kafkadatapath]))
        self.AppInfo['ProvisionerPath'] = kafkadatapath
    ...
    def setupStorage(self):
        ...
        print ('create Kafka Storage successfully')
        self.DataStorageObj.createSubFolder(self.AppInfo['KafkaDataPath'])
    ...
    def generateValues(self):
        self.AppInfo['KafkaImage'] = replaceDockerRepo(self.AppInfo['KafkaImage'], self.AppInfo['HarborAddr'])
    ...
    def applyYAML(self):
        ...
        if not self.k8sObj.checkNamespacedResourceHealth(name='kafka', namespace=self.AppInfo['Namespace'],
                                                         kind='StatefulSet'):
            try:
                self.k8sObj.deleteNamespacedStatefulSet(name='kafka', namespace=self.AppInfo['Namespace'])
            except:
                pass

            TmpResponse = self.k8sObj.createResourceFromYaml(filepath=os.path.join(TmpTargetNamespaceDIR, 'resource', 'kafka.yaml'),
        ...
        for itime in range(self.RetryTimes):
            TmpResponse = self.k8sObj.getNamespacedStatefulSet(name='kafka',
